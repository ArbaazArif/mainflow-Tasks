...
root.iconbitmap("C:\\Users\\rd\\Downloads\\icon.ico")
root.maxsize(600, 270)
root.minsize(600, 270)

image = Image.open("C:\\Users\\rd\\Downloads\\currency.png")
zoom = 0.5
...
def show_data():
    ...
    if amount == '':
        ...
        text.insert('end', f'{amount} USD Equals {cur_conv} {to_currency} \n\n Last Time Update --- \t{datetime.now()} ')
...
c1 = tk.StringVar()
c2 = tk.StringVar()
currchoosel = ttk.Combobox(root, width=20, textvariable=c1, state='randomly', font=('verdana', 10, 'bold'))

currchoosel['values'] = ('USD',)
currchoosel.place(x=300, y=40)
currchoosel.current(0)

E2 = Entry(root, width=20, borderwidth=1, font=('roboto', 10, 'bold'))
E2.place(x=20, y=80)

currchoosel2 = ttk.Combobox(root, width=20, textvariable=c2, state='readonly', font=('verdana', 10, 'bold'))
currchoosel2['values'] = ('All', 'AFN', 'ARS', 'AWG', 'AUD', 'AZN', 'BSD', 'BBD', 'BYN', 'BZD', 'BMD', 'BOB',
                          'BAM', 'BWP', 'BGN', 'KHR', 'CAD', 'KYD', 'CLP', 'CNY', 'COP', 'CRC', 'HRK', 'CUP',
                          'CZK', 'DKK', 'DOP', 'XCD', 'EGP', 'SVC', 'EUR', 'FKP', 'FJD', 'GHS', 'GIP', 'GTQ',
                          'GYD', 'HNL', 'HKD', 'HUF', 'ISK', 'INR', 'IDR', 'IRR', 'IMP', 'ILS', 'JMD', 'JPY',
                          'KZT', 'KPW', 'KRW', 'KGS', 'LAK', 'LBP', 'LRD', 'MKD', 'MYR', 'MUR', 'MXN', 'MNT',
                          'MZN', 'NAD', 'NPR', 'ANG', 'NZD', 'NIO', 'NGN', 'NOK', 'OMR', 'PKR', 'PAB', 'PYG',
                          'PEN', 'PHP', 'PLN', 'QAR', 'RON', 'RUB', 'SHP', 'SAR', 'RSD', 'SCR', 'SGD', 'SBD',
                          'SOS', 'ZAR', 'LKR', 'SEK', 'CHF', 'SRD', 'SYP', 'TWD', 'THB', 'TTD', 'TRY', 'TVD',
                          'UAH', 'GBP', 'UYU', 'UZS', 'VEF', 'VND', 'YER', 'ZWD',)

currchoosel2.place(x=300, y=80)
currchoosel2.current()
